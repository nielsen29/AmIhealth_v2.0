...
public class AdapterGlucosaGraficas extends RecyclerView.Adapter<AdapterGlucosaGraficas.ViewGrafHolder> {

    private ArrayList<MedidasGlucosaList> resultados = new ArrayList<>();
    private Context context;
    private ArrayList<LineData> DatosGraf = new ArrayList<>();
    private ArrayList<ArrayList<String>> fechas = new ArrayList<>();

    public AdapterGlucosaGraficas(ArrayList<MedidasGlucosaList> resultados, Context context) {
        ...
        */
        ...
        holder.graf.setOnChartValueSelectedListener(new OnChartValueSelectedListener() {
            ...
            public void onValueSelected(Entry e, Highlight h) {
                ...
                holder.graf.setMarker(new CustomGlucosaMarkerView(context,R.layout.graph_descrip_glucosa,resultados.get(position).getRealmResults().sort("datetime", Sort.DESCENDING).get((int) e.getX())));
            }
     ...
     */
    public ArrayList<LineData> getEntrys(ArrayList<MedidasGlucosaList> resultados){
        ...
        for (int i = 0; i < resultados.size() ; i++) {
            ...
            RealmResults<Glucosa> realmResult;
            ...
            for (int j = 0; j <realmResult.size() ; j++) {
                ...
                SYS.add(new Entry((float) j,(float) realmResult.get(j).getGlucosa()));
                //DIS.add(new Entry((float) j, (float) realmResult.get(j).getIca()));
...
*/
    ...
    public class ViewGrafHolder extends RecyclerView.ViewHolder implements OnChartValueSelectedListener {
        ...
        private ArrayList<Glucosa> listadoMedidias;
